...
function deleteProperty($id) {
    ...
    try {
        ...
        foreach ($images as $image) {
            $file_path = $_SERVER['DOCUMENT_ROOT'] . '/guntur-properties/' . $image['image_path'];
 ...
 * @param int $limit Number of inquiries to return (0 for all)
 * @param int $offset Offset for pagination
 * @return array Inquiries data
 */
...
function uploadFile($file, $destination, $allowed_extensions = [], $max_size = 0) {
    ...
    if (move_uploaded_file($file['tmp_name'], $filepath)) {
        return [
            ...
            'path' => str_replace($_SERVER['DOCUMENT_ROOT'] . '/guntur-properties/', '', $filepath)
        ];
    }
